...
def verify_post(section: str, raw_data: dict, post_text: str) -> VerificationResult:
    ...
    # Gate 2 is flag-only by default (GATE2_BLOCKS_POST = False in config).
    # This means posts are never hard-blocked because the AI verifier can't
    # parse a complex nested data structure. The human reviewer makes the call.
    if GATE2_BLOCKS_POST:
        blocked = not gate2.passed
    else:
        blocked = False

    # Flag if Gate 2 found any issue OR if confidence is LOW
    flagged = not gate2.passed or gate2.confidence == "LOW"
